<?php
/*
Plugin Name: spp-reviews
Plugin URI: 
Description: A handy plugin for WordPress developers to add reviews to their site. Enough functionality to be helpful and also stays out of your way.
Author: Anna Sliva
Author URI: 
Text Domain: 
...
{
	add_action( 'init', 'spp_reviews_init' );
	add_action( 'widgets_init', create_function('', 'return register_widget("SppReviewsWidget");') );
...
function spp_reviews_enqueue_scripts()
{
	...
	wp_enqueue_style( 'testimonial-rotator-style', plugins_url('/spp-reviews-style.css', __FILE__) ); 
...
{
	function SppReviewsWidget()
	{
		$widget_ops = array('classname' => 'SppReviewsWidget', 'description' => __('Displays rotating testimonials', 'testimonial_rotator') );
		$this->WP_Widget('SppReviewsWidget', __('Show Reviews', 'testimonial_rotator'), $widget_ops);
	}
